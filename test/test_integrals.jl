...
function test_readme_examples()
    ...
    @test isaccurate(integrate(cos, UnionDomain(0..1, 2..3))[1], 0.07329356604208204)
    @test isaccurate(integral(t -> sin(log(abs(t))), -1..1, LogSingPoint(0.0)), -1.0000000021051316)
    @test isaccurate(integral( x -> exp(log(abs(x[1]-x[2]))), (2..3) × (1..4), DiagonallySingular()),
2.333333333333333)
